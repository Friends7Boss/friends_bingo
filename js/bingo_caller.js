function playRecordedAudio(letter, number) {
    ...
    // Create audio objectsdocument.addEventListener('DOMContentLoaded', () => {
    ...
    function highlightNumbersRandomly() {
        ...
        function highlightNextNumber() {
            const blinkCount = 5; // Number of numbers to blink at once
            const blinkIndices = [];

            while (blinkIndices.length < blinkCount && highlightedIndices.size < numberElements.length) {
                let randomIndex;
                do {
                ...
                blinkIndices.push(randomIndex);
            }

            blinkIndices.forEach(index => {
                numberElements[index].classList.add('blink');
            });

            setTimeout(() => {
                blinkIndices.forEach(index => {
                    numberElements[index].classList.remove('blink');
                });
                highlightNextNumber();
            }, 200); // Blink duration
        }
